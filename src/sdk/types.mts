 ...
 *
 * - `tag`（缺省）：文字小方块，1–2 字。
 * - `ring`：一圈由两段弧组成的环，中间 1–2 字。给「一件事分两段、每段各有结论」用的
 *   ——考勤的上班卡 / 下班卡就是这个形状。
 *
 * 想画别的形状要先在宿主里加一档并说明它表达什么语义，**不要指望塞 SVG 或 emoji**：
 * 理由同 [`OverlayMark`]。
 */
export type BadgeShape = "tag" | "ring";

/**
 * 环上**一段弧**的语义档位。**不是颜色。**
 *
 * 与 [`OverlayTone`] 是两件事，别混用：`tone` 说的是「这条信息有多重要」，画出来是同一
 * 色相的深浅；这里说的是「这一段的结论是什么」，是一组互斥状态，宿主会用不同色相画它
 * ——把「打了卡」和「缺卡」画成同色相的深浅，等于要求用户记住哪个更深。
 *
 * | 档位 | 什么时候用 | 宿主画成 |
 * |---|---|---|
 * | `idle` | 还没有结论：卡还没打、时候还没到 | 中性灰 |
 * | `done` | 这一段成了 | 绿 |
 * | `warn` | 成了但有瑕疵：迟到、早退 | 琥珀 |
 * | `miss` | 缺了：缺卡 | 红 |
 * | `alt` | 被另一种情形取代：请假、调休 | 紫 |
 *
 * `alt` 与 `idle` 的分野是**有没有交代**：请假是有结论的（批过了），没打卡是没有结论。
 * 画成同一档会让「上午请假」看起来像「上午忘打卡」。
 */
export type BadgeArc = "idle" | "done" | "warn" | "miss" | "alt";

/**
 * 环形角标那两段弧。**固定两段。**
 *
 * 方向按**书写方向**而不是左右：`leading` 在 LTR 界面下是左半环，RTL 下自然翻过来。
 * 只给一段时另一段按 `idle` 算——「上午打了卡、下午还没到点」是最常见的一天。
 */
export interface BadgeArcs {
  /** 起段。考勤里是上班卡。 */
  leading?: BadgeArc;
  /** 收段。考勤里是下班卡。 */
  trailing?: BadgeArc;
...
export interface DayBadge {
   ...
   * **`shape: "ring"` 时截到 2 字符**——环的内径只有 10px 出头。
   */
  label: string;
  /**
   * 画成小方块还是环，缺省 `tag`。
   *
   * **不给这个字段等于沿用原来的小方块**，所以老插件不受影响。
   */
  shape?: BadgeShape;
  /**
   * 环的两段弧，**只在 `shape: "ring"` 时有意义**。
   *
   * `tag` 上给了会被宿主丢掉；`ring` 却没给时宿主补成两段 `idle`（一圈空环 + 中间那个
   * 字），不会降级成小方块——形态一变用户会以为是两种不同的东西。
   */
  arcs?: BadgeArcs;
