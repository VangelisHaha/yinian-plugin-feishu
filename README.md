...
一个插件，四件事，接进 [一念（Yinian）](https://github.com/VangelisHaha/nikou-agenda)：
...
- **要两次确认。** 勾上「飞书考勤」并授权之后，还得在**日历侧栏的「日历叠加」**里把
  它打开。这是刻意的：装插件与「让它读我的考勤」是两回事，关着的时候一念**根本不会去
...
- **不进一念的数据库、不参与多端同步、也不开放给 Agent 读。** 它是飞书的投影，一念不是
...
| 日历上一个考勤角标都没有 | 三件事按顺序查：勾了「飞书考勤」吗、重新授权过吗、**日历侧栏的「日历叠加」里打开了吗**。第三步最容易漏——关着的时候一念根本不会调插件 |
