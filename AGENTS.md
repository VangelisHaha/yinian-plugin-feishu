...
[安时（Nuncta）](https://github.com/VangelisHaha/nikou-agenda)的飞书插件：任务同步（双向）、日历与会议同步（pull-only）、考勤叠加层（只读，画在日历上）、通知渠道（出站）。基于[官方模板](https://github.com/VangelisHaha/yinian-plugin-template)，SDK 与 `doctor` 是从那里复制的。
...
契约是安时仓库的 `docs/11-plugin-architecture.md` §8.4。实现在
...
### 角标是一个环，不是一个字（0.6.1 起）

我们出的是 `shape: "ring"`：**左半环上班卡、右半环下班卡**，中间一个字说这天的性质。
上一版是文字小方块，「班」和「加」在格子里长得几乎一样，而「上午打了卡下午忘打」只能
靠悬浮看——一天里最该被一眼看到的恰好是三件事，一两个字装不下。

- **每段弧给的是语义档位，不是颜色**：`idle`（还没有结论）/ `done`（成了）/ `warn`
  （迟到早退）/ `miss`（缺卡）/ `alt`（请假）。色值全在宿主的 `--yn-punch-*` 里。
- **中间那个字只有三种**：`班` / `加` / `假`（英文 `W` / `OT` / `LV`）。原来的「缺」「异」
  **删掉了**——状态由弧的颜色说，让字也说一遍就是同一件事画两遍，而两处一旦不一致
  （「班」配一圈红弧）用户只会觉得其中一个是错的。
- **环内径只放得下 1 个汉字**（宿主截到 2 字符，`tag` 是 4），所以别想往字里塞状态。

要画这个环，`AttendanceDay` 多了三个字段（缓存形状变了，`CACHE_VERSION` 已升到 2）：

- `punchIn` / `punchOut`：**那一次**打卡的结论（`normal|late|early|lack|todo`）。
  `issues: ["lack"]` 说不出缺的是上班卡还是下班卡，而这恰恰是最常见的异常。
  一天多段班次时 `punchStatus()` 取**最差**的那一次——取第一条会让「上午正常、下午缺卡」
  显示成正常。`PUNCH_RESULT` 表外的枚举（`NoNeedCheck` 以及飞书将来加的）一律当
  「无需打卡」而**不是缺卡**：加一个枚举不该让日历上冒出一片红环。
- `leavePeriod`：`am` / `pm` / `full`。**这不是推断出来的**——飞书的
  `check_in_result_supplement` / `check_out_result_supplement` 上下两侧本来就是分开的，
  只有上班侧是 `Leave` 就是上午假。上一版只判「有没有请假」，把这个信息丢了，于是
  「上午请假下午上班」和「全天请假」在日历上长得一模一样。
...
的今天），有排班无打卡时按 `date < today` 分成 `absent` 与 `scheduled`。

`scheduled` **只画今天那一格**（0.6.1 改口，原来一律不画）：今天恰恰是最值得看的一格
——上班卡绿了、下班卡还灰着；而未来一整月的空环没有信息量，哪些天上班日历自己就知道。
之所以现在敢画，是因为环有 `idle` 这一档，说得出「还没有结论」；上一版只有一个「班」字，
画出来就是撒谎。两种情况都**不计入任何统计**。
...
之前会先减 1 秒再取日期），安时契约要的也是右开，所以**原样传，不要加减一天**。
...
补历史任务时必须用安时记录的真实完成时间。用 `Date.now()` 会把所有历史任务的完成时间改成今天，用户的完成记录就毁了，而且**不可逆**。
...
2. 首次同步后飞书日程出现在安时日历，**重复日程的每一次都在**，全天日程占的天数正确；
3. 带视频会议的日程，详情「来源」区有可点的会议链接；
4. 外部日历在安时里改不动（只读），飞书侧改时间后下一轮同步跟着变；
5. 在飞书里取消一个已同步的日程 → 安时侧变成「已取消」而不是消失；
6. 插件设置里点「发送测试通知」能收到；关掉安时的「飞书」渠道开关后真实提醒不再发到飞书；
...
4. **点「在安时中打开」能唤起安时并打开对应事项**——这一条最可能失败：飞书客户端不
   一定放行 `yinian://` 这类自定义 scheme。不放行就把 `card.mts` 的 `openButton`
   去掉，别留一个点不动的按钮；
5. 安时没在运行时点那个按钮，应用应该被启动并直接打开那个事项（冷启动路径）；
...
**考勤叠加层（0.5.0 起；0.6.1 换成环形角标）**

1. 勾上「飞书考勤」并重新授权 → 「查看授权状态」里它进「已生效」；
2. 打开日历侧栏的「日历叠加」→ 里面出现「飞书考勤」这一项，**默认是关着的**；
3. 打开它 → 月视图格子右上角出现**环形角标**：中间「班」，正常出勤的日子**左右两段
   都是绿的**；周末加班中间是「加」，全天请假中间是「假」且整圈紫；
4. **只缺了一次卡的那天要能一眼看出缺的是哪一次**：缺的那半环是红的、另一半照常。
   这是环形存在的全部理由，看不出来就说明 `punchIn` / `punchOut` 没落地；
5. **半天假那天中间仍然是「班」**（人确实来上班了），只有一段是紫的，悬浮说明里写
   「上午请假」/「下午请假」；
6. **今天那一格是活的**：上午打过卡后左半环绿、右半环还灰着；**未来的工作日一个环都
   没有**（未来一片空环没有信息量）；
7. 悬浮角标能看到打卡时间（`打卡 09:02–18:12`）；
8. **切深色主题再看一遍**：五档颜色都要还能分辨，尤其绿别糊成黑；
9. 侧栏「负载」卡里多出出勤 / 加班 / 请假几行，**数字是当月口径而不是那 42 格**
   （拿 8 月 31 日到 10 月 11 日算会多几天，肉眼对不出来，用「出勤天数 = 本月工作日数」
   自己核一遍）；
10. 关掉那个开关 → 角标与那几行一起消失，**且插件日志里不再出现考勤刷新记录**
    （关着时宿主根本不调，这是授权闸门而不是显示开关）；
11. 把「普通工作日也打角标」关掉 → 只剩加班 / 请假 / 异常那几天；
12. 断网翻月 → 角标照常显示（读的是缓存），日志里有一条刷新失败但界面无异常。
    **首次装上 0.6.1 时缓存会整份重拉**（`CACHE_VERSION` 1→2），所以第一次翻月可能空
    几秒——这是预期行为，不是坏了。

**任务**

1. 填凭据 → 开始授权 → 浏览器同意 → 检查授权显示「已授权为 XXX」；
2. 首次同步后飞书任务出现在安时，跨天任务的时间轴跨度正确；
3. 在安时里勾完一个任务 → 飞书侧变成已完成，且 60 秒内的轮询没有把它拍回未完成；
4. 在飞书里勾完一个任务 → 下一轮同步后安时侧变成已完成，完成时间是飞书的真实时间；
5. 改标题 → 飞书侧标题同步变化；
